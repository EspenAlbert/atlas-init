...
class PrivateEndpoint:
    ...
    def __post_init__(self):
        if self.endpoints is not None:
            self.endpoints = [Endpoint(**x) if not isinstance(x, Endpoint) else x for x in self.endpoints]
    ...
    def __post_init__(self):
        if self.private_endpoint is not None:
            self.private_endpoint = [
                PrivateEndpoint(**x) if not isinstance(x, PrivateEndpoint) else x for x in self.private_endpoint
            ]
...
class ReplicationSpec:
    ...
    def __post_init__(self):
        if self.region_configs is not None:
            self.region_configs = [
                RegionConfig(**x) if not isinstance(x, RegionConfig) else x for x in self.region_configs
            ]
    ...
    def __post_init__(self):
        ...
        if self.replication_specs is not None:
            self.replication_specs = [
                ReplicationSpec(**x) if not isinstance(x, ReplicationSpec) else x for x in self.replication_specs
            ]
...
class ResourceExt(Resource):
    electable: Optional[CustomSpec] = None
...
def default_region_configs(spec: CustomSpec) -> list[RegionConfig]:
    return [
        RegionConfig(
            priority=8 - i,
    ...
    for spec in specs:
        ...
        if auto_scaling_compute:
            for region_config in spec.region_configs:
                region_config.auto_scaling = Autoscaling(
