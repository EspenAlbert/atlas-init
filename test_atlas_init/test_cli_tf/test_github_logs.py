...
def test_workflow_with_multiple_runs():
    ...
    for job in paginated_jobs:
        if not is_test_job(job.name):
            print(f"non test job: {job.name}, attempt {job.run_attempt}, {job.created_at}")
            continue
        print(f"found test job: {job.name}, attempt {job.run_attempt}, {job.created_at}, url: {job.html_url}")


def test_is_test_job():
    assert is_test_job("tests-1.8.x-latest / tests-1.8.x-latest-dev / config")
    assert not is_test_job("tests-1.8.x-latest / tests-1.8.x-latest-dev / get-provider-version")
    assert not is_test_job("clean-after / cleanup-test-env-general")
    assert not is_test_job("clean-before / cleanup-test-env-qa")


@pytest.mark.skipif(os.environ.get("JOB_LOGS_PATH", "") == "", reason="needs os.environ[JOB_LOGS_PATH]")
