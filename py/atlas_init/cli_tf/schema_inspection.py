...
def log_optional_only(repo_path: Path):
    for resource_path in (repo_path / "internal").rglob("resource_*.go"):
        for name in schema_attributes_plugin_framework(
            resource_path, optional=True, computed=False
        ):
            logger.info(f"found optional-only attr={name} in {resource_path}")


def schema_attributes_plugin_framework(
    path: Path, optional: bool = False, computed: bool = False
) -> Iterable[str]:
    text = path.read_text()
    if "github.com/hashicorp/terraform-plugin-framework/resource/schema" not in text:
        return
    for attr in iterate_schema_attributes(
        text, "MapAttribute", "ListAttribute"
    ):
...
def as_regex(attribute_type: str) -> re.Pattern:
    return re.compile(
        r'"(?P<name>[\w_]+)": (?P<attribute_type>schema\.%s)\{(?P<brackets>[^}]+)\}'
        % attribute_type,
        re.MULTILINE,
    )
