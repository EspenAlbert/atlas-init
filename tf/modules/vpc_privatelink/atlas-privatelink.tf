...
output "info" {
  value = {
    ...
    vpc_endpoint_id = aws_vpc_endpoint.vpce_east.id
  }
}

output "env_vars" {
  value = {
    MONGODB_ATLAS_PRIVATE_ENDPOINT_ID=aws_vpc_endpoint.vpce_east.id
    MONGODB_ATLAS_PRIVATE_ENDPOINT_DNS_NAME=aws_vpc_endpoint.vpce_east.dns_entry[0].dns_name
  }
}